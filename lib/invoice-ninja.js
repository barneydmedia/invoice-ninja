...
function generateItemList(items, currencyFormat, template) {
  ...
  if (items) {
    items.forEach(function(element) {
      ...
      // calculate missing numbers if left blank
      if ((typeof(element.quantity) == 'undefined' || element.quantity.length === 0) && (element.amount > 0 && element.rate > 0)) {
        element.quantity = element.amount / element.rate;
      }
      if ((typeof(element.rate) == 'undefined' || element.rate.length === 0) && (element.quantity > 0 && element.amount > 0)) {
        element.rate = element.amount / element.quantity;
      }
      if ((typeof(element.amount) == 'undefined' || element.amount.length === 0) && (element.quantity > 0 && element.rate > 0)) {
